...
**Resolved by [ADR-0016](0016-a-rewind-re-renders-the-day-it-changed.md).** They do — a Claude Code resume appends to the same file too, and `claude.py` splits by day the same way. What did not transfer is the stability argument above: this record's "an unchanged cursor means `cards.write` never opens the file" still holds, but its unstated premise — that a day's records can never change once written — is a property of an append-only log, and Claude Code writes a tree that a rewind can re-decide retroactively. ADR-0016 is what happens then.
