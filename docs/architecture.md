...
`.crate/templates/` and `.claude/commands/` are the two things in a vault the engine owns rather than
you: `crate init` installs them and `crate upgrade` refreshes them, while everything else is left
alone. Slash commands have to be on disk for Claude Code to find them, which is what makes a vault
not quite pure content — see [ADR-0009](adr/0009-engine-owned-vault-files.md).
