...
One file is a thread, and a thread can be resumed indefinitely — so the Codex adapter splits it by local day, and each day the thread was actually active on becomes its own card ([ADR-0015](adr/0015-a-day-of-a-thread-is-a-card.md)). Each card is dated to its own day, carries the branch and CLI version in force *that* day, and reports that day's duration, so `crate day` lists a fortnight-long thread under every day it touched and under none of the dormant ones. A day earns a card on the same test a whole file does — its records have to yield at least one turn — so a resume whose only content is replayed history produces nothing. The split happens at capture, before turns are built, and each day's cursor is that day's own record count: appending Wednesday to a rollout writes Wednesday's card and leaves Monday's and Tuesday's byte-identical and unopened, which is what `raw/` being immutable requires.
