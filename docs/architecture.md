...
**Codex** (`~/.codex/sessions/<Y>/<M>/<D>/rollout-*.jsonl`) is the counter-example that proves the model generalizes: a flat append-only log with no tree and no rewinds, where a command is a `function_call` and a file edit is an `apply_patch` whose path lives inside a patch string. Its adapter is a linear scan rather than a tree walk, keeps and drops the same *kinds* of thing, and emits the same card. The structure differs but the shape recurs — a Codex resume appends to the same rollout file, re-emitting an identical `session_meta` rather than starting a new one, so one file is still one session and one card; and its subagents (the `guardian` auto-approver) are their own files marked `thread_source: subagent`, the equivalent of Claude's sidechains and dropped the same way. `parent_thread_id` links a subagent to the session that spawned it and is never a resume link, which the name invites you to assume it is.

That leaves one known limitation, since a thread can be resumed indefinitely into the same file: a card is dated to the day the thread *started*, carries the branch and CLI version it started on, and reports a duration spanning everything since. A thread resumed across a fortnight is one card on day one, and `crate day` won't list it under any later day it touched.
